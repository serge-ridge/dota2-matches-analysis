...
def get_data(f_name, show_gaps=False):
    ...
    '''
    # this test had not effect to the final score
    # sum teams experience
    # init id list for sum
    s_id_lst = ['xp', 'gold', 'lh', 'kills', 'deaths', 'items']
    for f in s_id_lst:
        features_f['r_%s' % f] = 0
        features_f['d_%s' % f] = 0
        for p in range(1, 6):
            # add (p) teams members stats to team columns
            features_f['r_%s' % f] += features_f['r%d_%s' % (p, f)]
            features_f['d_%s' % f] += features_f['d%d_%s' % (p, f)]
    '''
...
features, y = get_data('features.csv', show_gaps=False)
X = features.values
# KFold init
...
for n_estimators in trees_lst:
    ...
    gbc = GradientBoostingClassifier(
        verbose=True, n_estimators=n_estimators,
        random_state=241, max_depth=7)
...
'''
X = bag_of_words(features, use_bag=True, drop_features=True)
# standardize X
X = standardize(X)
# init Logistic Regression with best C
lr = LogisticRegression(random_state=241, C=0.01, solver='lbfgs')
...
print(y_min, y_max)
y_s = pd.DataFrame(data={'radiant_win': y_pred[:, 1]}, index=features.index)
y_s.to_csv('predict_radiant_win.csv')
